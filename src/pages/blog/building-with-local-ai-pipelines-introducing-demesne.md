...
Getting to “Repository Editor” comes for free as soon as I install Claude Code. “Connected Agent” is pretty easy, just add MCP servers and skills (and try not to go too crazy on the skills).

But I found getting to “Agent Orchestrator” going well requires some real work. Dynamic workflows in Claude Code and similar are getting there, but you quickly run into the next problem...
...
**Let agents be agents**: Nothing programmatically enforces workflow structure. If something goes wrong, the agent can change plan mid-pipeline. Any subagent can decide it wants to launch more subagents. I make the bet that already, and more over time, letting the agents use their intelligence rather than binding them tightly will result in on average better behaviour. This bet is a lot easier to make when the sandboxes are running locally and off agent subscriptions, and the worst resource exhaustion can do is eat all my RAM and use up my five hour quota, instead of running on someone else’s cloud somewhere.[^1]
...
[^2]: Update 2026-07-14: I have now switched over to primarily using the new GPT-5.6 models for sandboxes, so this has borne fruit.

[^3]: See there was a rationale behind the name.
